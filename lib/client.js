		...
		if (texts.length > 0) {
			Promise.all(texts).then(function (parts) {
				...
				if (!insertIntoDraft(blocks.join("\n\n"))) {
					console.warn("[dsh-image-picker] composer input not found — dumping DOM for diagnosis:");
					console.warn("[dsh-image-picker]   composer-card:", !!document.querySelector("[data-composer-card]"));
					console.warn("[dsh-image-picker]   composer-input:", !!document.querySelector("[data-composer-input]"));
					console.warn("[dsh-image-picker]   card contenteditable:", !!document.querySelector("[data-composer-card] [contenteditable='true']"));
					console.warn("[dsh-image-picker]   global contenteditable:", !!document.querySelector("[contenteditable='true']"));
					console.warn("[dsh-image-picker]   card textarea:", !!document.querySelector("[data-composer-card] textarea"));
				}
